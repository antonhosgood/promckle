...
class DataLoader:
    ...
    This class provides methods to read (multiple) blocks and export the data to
    various formats.
    ...
    def read_block(self, path: Union[str, bytes, os.PathLike]) -> None:
    ...
    def to_parquet(self, path: Union[str, bytes, os.PathLike]) -> None:
        """Exports the read data to Parquet format."""
        path = get_absolute_path(path)
        self.to_dataframe().to_parquet(path)

    def to_tsv(self, path: Union[str, bytes, os.PathLike]) -> None:
